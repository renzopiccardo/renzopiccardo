document.addEventListener("DOMContentLoaded", function () {
    ...
    leerJSONLocal('json.json', function (text) {
        ...
        /*Creo un li por cada juego en el JSON*/
        /*Dentro de cada li estan los elementos correspondientes a cada juego como titulo, foto, descripcion, precio y un href particular para utilizar el get de parametros*/
...
{
    var list, i, switching, b, haceSwitch, direccion, cantidadSwitch = 0;
    list = document.getElementById("ulJuegos");
    switching = true;
    // orden ascendente
    direccion = "asc";
    ...
    while (switching) 
    {
        ...
        else 
        {
            ...
            if (cantidadSwitch == 0 && direccion == "asc") 
            {
                direccion = "desc";
                switching = true;
            }
...
function vistaCompacta() 
{
    ...
    var precio = document.getElementsByClassName("tag");

    for (var i = 0; i < fig.length; i++) 
    {
        fig[i].style.display = 'none';
        p[i].style.display = 'none';
        precio[i].style.display = 'none';
    }
}
